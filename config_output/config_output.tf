...
resource "camc_scriptpackage" "get_cluster_config" {
  ...
  program = split(
    ",",
    length(var.openshift_cluster_name) > 0 ? join(
      ",",
      [
        "sudo /bin/bash /tmp/gather_output.sh -c ${var.openshift_cluster_name} -as ${var.openshift_server} -ap ${var.openshift_port} -u ${var.openshift_user}",
      ],
      ) : join(
      ",",
      [
        "sudo /bin/bash /tmp/gather_output.sh -as ${var.openshift_server} -ap ${var.openshift_port} -u ${var.openshift_user}",
      ],
    ),
  )
  on_create           = true
...
resource "null_resource" "output_create_finished" {
  depends_on = [
    camc_scriptpackage.get_cluster_config,
    null_resource.config_output_dependsOn,
  ]
  provisioner "local-exec" {
